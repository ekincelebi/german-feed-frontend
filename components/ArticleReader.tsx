  ...
  const handleExplainClick = async () => {
    ...
    try {
      const response = await fetch('/api/explain', {
        ...
        body: JSON.stringify({
          text: articleContent,
          phrases: highlightsNeedingExplanations.map(h => h.text.trim())
        })
      })
      ...
      const updatedHighlights = highlights.map(h => {
        if (!h.explanation) {
          // Match using trimmed text for comparison
          const exp = explanations.find((e: any) => e.phrase.trim() === h.text.trim())
          if (exp) {
      ...
      // Show warning if there are still highlights without explanations
      const stillPending = updatedHighlights.filter(h => !h.explanation).length
      setShowPendingWarning(stillPending > 0)
    } catch (error) {
            ...
            {showExplanations ? (
              <>
                <div className="p-6 border-b border-gray-200 bg-gray-50">
                  ...
                  {showPendingWarning && highlights.filter(h => !h.explanation).length > 0 && (
                    <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2">
                      <svg className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                      </svg>
                      <div className="text-xs text-amber-800">
                        <p className="font-semibold mb-1">Some highlights couldn't be explained</p>
                        <p className="text-amber-700">This happens when only part of a word is selected. Please remove and re-highlight the full word or phrase.</p>
                      </div>
                    </div>
                  )}
                </div>
